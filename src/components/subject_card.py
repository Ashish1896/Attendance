...
def subject_card(name, code, section, stats=None, footer_callback=None):
    ...
    if stats:
        ...
        for icon, label, value in stats:
            html+= f'<div style="background: #EB459E15; color: #000000; font-weight: 600; padding:6px 14px; border-radius:12px; font-size:0.95rem; border: 1px solid #EB459E30;">{icon} <b>{value}</b> {label} </div>'
